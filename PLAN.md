...
* DONE 2026-07-20 — Beat 4 fingerprint rebuilt as a "swelling river of accumulation." Connor's critique of the old tributaries: too thin, tons of unused negative space, and it didn't pay off "how things added up over time" (it encoded time + convergence but not magnitude). Root cause: quantity wasn't mapped to anything. Fix: a streamgraph where thickness = cumulative scale — each chapter is a band that joins at its start year and persists/stacks, so the river grows from a 2018 trickle to a full 2026 body (BCG swells hardest as its workstreams/value compound). Internal filaments for body, gold emerges toward now, subtle edge feeders use the negative space; crisp stacked-area chart at Logic → painterly river at Intuition. Follow-ups to taste: push gold harder at the far-right end; richer fills at high t.
* PARKED — Fluid (studio) flowing left→right: Connor's standing preference is for the liquid flow to travel L→R (like a river). Easy pass: bias the velocity field rightward + move pigment sources to the left edge so paint enters left and streams right. Do whenever.
